...
for L in [small_L,large_L]:
    ...
    for i,Delta_1 in enumerate(Delta_1_list):
        ...
        for j,W in enumerate(W_list):
            ...
            if j == 0:
                if L == small_L:
                    ax.axhline(y=finite_size_eq,color='k',ls=':',label=r'$\frac{1}{4(%i)}$' % L)
                else:
                    ax.axhline(y=finite_size_eq,color='k',ls='--',label=r'$\frac{1}{4(%i)}$' % L)
            if W < 0.0001:
                if L == small_L:
                    ...
                    ax.plot(t[np.logspace(np.log10(first_index),np.log10(last_value),num_xs,dtype=int)],np.mean(same_site_data[:,np.logspace(np.log10(first_index),np.log10(last_value),num_xs,dtype=int)],axis=0),'x',label=r'$L=%i$ \& $W=%.1f$' % (L,W),color=color_list[j],markersize=12,mew=2)
                else:
                    ax.plot(t[first_index:last_value],same_site_data[0,first_index:last_value],label=r'$L=%i$ \& $W=%.1f$' % (L,W), color = color_list[j])
                ...
                if  L == small_L:
                    ax.plot(t[np.logspace(np.log10(first_index),np.log10(last_value),num_xs,dtype=int)],average_data[np.logspace(np.log10(first_index),np.log10(last_value),num_xs,dtype=int)],'x',label=r'$L=%i$ \& $W=%.1f$' % (L,W),color=color_list[j],markersize=12,mew=2)
                else:
                    std = np.std(same_site_data,axis=0)
                    sem = std/np.sqrt(total_num_runs)
                    ax.plot(t[first_index:last_value],average_data[first_index:last_value],label=r'$L=%i$ \& $W=%.1f$' % (L,W),color=color_list[j])
...
filename = os.path.join(fig_dir,'random_potential_spin_spin_same_site_%iruns.pdf' % total_num_runs)
